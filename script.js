...
function validarCampos(){
  ...
  var longnombre = validarLongitud(nombre);
  var longmail = validarLongitud(mail);
  var pricar = validarPrimerCaracter(mail);
  var arroba = validarArroba(mail);
  var dominio = validarDominio(mail);
  flag = true;
  ...
  if (longmail && pricar && arroba && dominio){
      // No pasa nada

  }else{
      var error1 = document.createElement("li");
      error1.innerHTML = "El email no es valido";
...
function validarLongitudEmail(correo){
  var n = correo.length;
  if (n >= 6){
      return true;
  } else {
...
function marcarCompletado(index){
  contactos[index].completado = !contactos[index].completado;
  imprimirTodosLosContactos();
}
